...
// ── HOME GUILD — ALWAYS FULLY UNLOCKED ────────────────────────────────
const HOME_GUILD_ID = process.env.HOME_GUILD_ID || '1438103556610723922';
function isHomeGuild(guildId) { return String(guildId) === String(HOME_GUILD_ID); }
function hasPremiumAccess(guildId) { return isHomeGuild(guildId); }
...
function mkTrack(yt, url = null, src = "youtube", thumb = null) {
  const id = yt?.id || yt?.videoId || yt?.video_id || null;
  ...
  if (!isValidHttpUrl(cleanUrl)) return null;

  return {
    title: yt?.title || "Unknown Track",
    url: cleanUrl,
    duration: yt?.durationInSec ?? yt?.duration ?? 0,
    thumbnail: thumb || yt?.thumbnails?.[0]?.url || yt?.thumbnail || null,
    source: src,
    ytId: id,
    addedAt: Date.now(),
  };
...
console.log("[Engine] resource started:", {
  title: track.title,
...
// ── LAUNCHPAD PANEL ────────────────────────────────────────────────────
function buildLaunchpadEmbed(state, guild) {
  ...
  return new EmbedBuilder()
    .setColor(color)
    .setAuthor({ name: '🌈 CONbot5 OS — Prismatic Radiance Matrix', iconURL: FT.iconURL })
    .setTitle('Discord-native music · social listening · audio lab')
    .addFields(
      { name: '📡 Status',      value: conn ? '🟢 Online' : '🔴 Offline', inline: true },
      { name: '🎵 Session',     value: state.mood ? `${mood?.emoji} ${mood?.label}` : (state.autoplay ? '🤖 AutoPlay' : '🎛️ Manual'), inline: true },
      { name: '🎙️ Voice',       value: state.voiceChannelId ? '🟢 Connected' : '⚪ Disconnected', inline: true },
      { name: '📋 Queue',       value: String(state.queue.length), inline: true },
      { name: '▶️ Now Playing', value: state.current ? state.current.title.slice(0,60) : '_Idle_', inline: true },
      { name: '🔊 Volume',      value: `${state.volume}%`, inline: true },
    )
    .setFooter({ ...FT, text: FT.text + ' · Buttons below — no slash commands needed after setup' })
    .setTimestamp();
}
...
function buildLaunchpadComponents(state) {
  ...
  const hasQ    = state.queue.length > 0 || !!state.current;
  const r1 = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId('c5_launch_session').setLabel('▶ Start Session').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId('c5_launch_search') .setLabel('⌕ Search / Play').setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId('c5_launch_np')     .setLabel('🎵 Now Playing')  .setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('c5_queue')         .setLabel('📋 Queue')        .setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('c5_browse')        .setLabel('🎸 Browse')       .setStyle(ButtonStyle.Secondary),
  );
  const r2 = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId('c5_stop')          .setEmoji('⏹️').setStyle(ButtonStyle.Danger).setDisabled(!hasQ),
    new ButtonBuilder().setCustomId('c5_autoplay')      .setLabel('🤖 Auto').setStyle(state.autoplay ? ButtonStyle.Success : ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('c5_launch_refresh').setEmoji('🔄').setStyle(ButtonStyle.Secondary),
  );
  const r3 = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder().setCustomId('c5_mood')
...
async function cmdLaunchpad(i, client) {
  const state = getState(i.guildId);
  state.textChannelId = i.channelId;
  const msg = await i.editReply({ embeds: [buildLaunchpadEmbed(state, i.guild)], components: buildLaunchpadComponents(state) });
  state.launchpadMsgId = msg.id;
  state.launchpadChId  = i.channelId;
}

async function handleLaunchpadButton(id, i, client) {
  const state = getState(i.guildId);
  if (id === 'c5_launch_session') {
    const vc = i.member?.voice?.channel;
    if (!vc) { try { await i.followUp({ content: '⚠️ Join a voice channel first!', ephemeral: true }); } catch {} return; }
    state.textChannelId = i.channelId;
    await ensureVC(state, vc, client).catch(() => {});
    if (!state.current && state.queue.length) await playNext(state, client);
    try { await i.followUp({ content: '🟢 Session started! Use `/play` or pick a Mood Room.', ephemeral: true }); } catch {}
  } else if (id === 'c5_launch_search') {
    try { await i.followUp({ content: 'Use `/play <song name>` to search.', ephemeral: true }); } catch {}
  } else if (id === 'c5_launch_np') {
    try { await i.followUp({ embeds: [buildNowPlayingEmbed(state, 0)], ephemeral: true }); } catch {}
  }
  // c5_launch_refresh falls through to updateDashboard in handleButton caller
}

// ── DIAGNOSE VOICE ─────────────────────────────────────────────────────
async function cmdDiagnoseVoice(i, client) {
  const state = getState(i.guildId);
  const conn  = getVoiceConnection(i.guildId);
  const url   = state.current?.url || null;
  let ytValid = null;
  if (url) { try { ytValid = await playdl.validate(url); } catch (e) { ytValid = `error: ${e.message}`; } }

  const emb = new EmbedBuilder()
    .setColor(conn ? 0x35FF9E : 0xFF4B4B)
    .setTitle('🔬 CONbot5 Voice Diagnostics')
    .addFields(
      { name: '🆔 Guild',         value: i.guildId,                                         inline: true  },
      { name: '🎙️ Voice Ch',      value: state.voiceChannelId || '_Not set_',              inline: true  },
      { name: '📡 In Voice',      value: conn ? '✅ Yes' : '❌ No',                          inline: true  },
      { name: '🔌 Conn Status',   value: conn?.state?.status || '_None_',                  inline: true  },
      { name: '🎵 Player',        value: state.player?.state?.status || '_None_',          inline: true  },
      { name: '🔊 Volume',        value: `${state.volume}%`,                                inline: true  },
      { name: '📋 Queue',         value: String(state.queue.length),                        inline: true  },
      { name: '🎶 Now Playing',   value: state.current?.title?.slice(0,60) || '_Nothing_', inline: false },
      { name: '🔗 URL',           value: url ? `\`${url.slice(0,80)}\`` : '_None_',         inline: false },
      { name: '✅ play-dl Valid', value: ytValid ? String(ytValid) : '_No track_',          inline: true  },
      { name: '📡 API',           value: process.env.API_URL ? '✅ Set' : '⚠️ Missing',     inline: true  },
    )
    .setFooter(FT).setTimestamp();

  if (!conn) emb.setDescription('⚠️ **Not in voice.** Run `/play <song>` or click Start Session.');
  else if (state.player?.state?.status !== AudioPlayerStatus.Playing) emb.setDescription('⚠️ **Connected but not playing.** Check: server mute · bot volume · YOUTUBE_COOKIE');
  else emb.setDescription('✅ Bot is connected and playing audio.');
...
buildSmartMixComponents,
  ...
  isLaunchpadButton: id => ['c5_launch_session','c5_launch_search','c5_launch_np','c5_launch_refresh'].includes(id),
};
